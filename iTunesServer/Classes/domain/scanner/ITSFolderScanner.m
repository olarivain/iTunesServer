...
- (void) setScannedPath:(NSString *)aPath
{
	...
	{
		DDLogInfo(@"Path %@ is not an existing folder.", aPath);
		return;
	}
...
- (void) moveFolderItems
{
	...
	for(ITSFolderItem *item in movableItems)
	{
		...
		if(error)
		{
			DDLogInfo(@"*** FATAL *** Could not move item %@", item.itemId);
...
- (void) start
{
	...
	{
		DDLogInfo(@"*** FATAL no destination path for auto import");
		return;
	}
	...
	DDLogInfo(@"Starting folder scanner at path: %@", path);
